...
/// Sums credited hydration ml per day from [entries], keyed by [dateKey] —
/// coffee and tea count at their reduced factor, so the trend chart matches
/// what the daily goal is measured against. Pure and side-effect free —
/// [entries] is expected to already be the retained history (see
/// [HydrationService]'s retention window).
Map<String, int> bucketWaterByDay(List<WaterEntry> entries) {
  final buckets = <String, int>{};
  for (final entry in entries) {
    final key = dateKey(entry.timestamp);
    buckets[key] = (buckets[key] ?? 0) + entry.hydrationMl;
  }
  return buckets;
}
